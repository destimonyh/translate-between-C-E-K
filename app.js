...
(function () {
  ...
  $('vocabList').addEventListener('click', function (e) {
    ...
    userData.vocab = userData.vocab.filter(function (x) { return String(x.id) !== String(id); });
  ...
  $('addForm').addEventListener('submit', function (e) {
    ...
    var entry = {
      ...
      zh: $('a_zh').value.trim(),
      en: $('a_en').value.trim(),
      koRom: $('a_koRom').value.trim() || '需验证',
      koMean: $('a_koMean').value.trim() || '需验证',
      domain: $('a_domain').value,
      level: $('a_level').value,
      note: $('a_note').value.trim(),
      conf: ($('a_koRom').value.trim() && $('a_koMean').value.trim()) ? 'verified' : 'needs-check',
      addedAt: new Date().toISOString()
    };
    if (!entry.zh || !entry.en || !entry.note) { setStatus('addStatus', '请填写中文/英文/解析', 'err'); return; }
    userData.vocab.push(entry);
    persistAndRender('新增词条: ' + entry.zh, 'addStatus', function () {
      $('addForm').reset(); $('a_level').value = '核心';
    });
  });
  ...
  function renderNotes() {
    ...
    list.slice().reverse().forEach(function (n) {
      ...
      el.innerHTML =
        ...
        '<div class="nd">' + esc(n.addedAt || '') + '</div>';
      box.appendChild(el);
    });
  }
  ...
  $('noteForm').addEventListener('submit', function (e) {
    ...
    var note = {
      ...
      cat: $('n_cat').value.trim(),
      body: $('n_body').value.trim(),
      tags: $('n_tags').value.trim(),
      addedAt: new Date().toISOString()
    };
    if (!note.title || !note.body) { setStatus('noteStatus', '请填写标题与正文', 'err'); return; }
    userData.notes.push(note);
    persistAndRender('新增笔记: ' + note.title, 'noteStatus', function () { $('noteForm').reset(); });
  });
